(() => {
  ...
  const renderNavbar = () => {
    ...
    const hideAuthForLanding = !loggedIn && currentRoute === '/';

    const authLinksHtml = hideAuthForLanding
      ? ''
      : loggedIn
          ? `<button data-logout class="px-5 py-2 bg-red-600 text-white text-sm font-semibold rounded-lg transition-all duration-300 hover:bg-red-700 hover:shadow-lg hover:shadow-red-600/40">Logout</button>`
          : `
    ...
    const mobileAuthHtml = hideAuthForLanding
      ? ''
      : loggedIn
    ...
    logoutButtons.forEach((button) => {
      button.addEventListener('click', () => {
        logout();
        window.location.href = routeToFile['/'];
      });
    });
  };
